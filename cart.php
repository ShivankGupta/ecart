...
if(!empty($_SESSION['cart']))
{
   ...
   $q="select * from shop where item_id in(";
   	foreach($_SESSION['cart'] as $id=>$value)
   		{$q .=$id.',';}
   	$q=substr($q, 0, -1).') order by item_desc ASC';
   ...
   while($row=mysqli_fetch_array($r,MYSQLI_ASSOC))
   {
         $subtotal=$_SESSION['cart'][$row['item_id']]['quantity']
          ...
          mysqli_close($dbc);}
          else{echo '<p ><h1 style="color:red">Your Cart Is Currently Empty.</h1></p>';}
     ...
     ?>
     <html>
     <body style= "background-image:url('image/cajpg');background-repeat:repeat">